...
# references:
# code and solutions provided on course github at https://github.com/jh2343/590-CODES
...
alpha		=	0.25	#MOMENTUM PARAMETER ... exponential decay factor
...
model_type 	=	"linear";  
## model_type 	=	"logistic";  	 
## model_type 	=	"ANN";  		  # ******** SELECT ANN MODEL
##------------------------------------------------------------------------
...
df = pd.read_csv('mpg.csv', header = 0, names=column_names,
                          na_values='?', comment='\t',
...
#------------------------
# TAKES THESE SUBMATRICES AND EVALUATES THE ANN
# VIA np.matmul AND LOOP OVER THE LAYERS
# a single for loop over the even indices of the submatrices array
# and a few matrix multiplications. 
# Using the "multiple input" implementation the matrices 
# will need to be transposed (see lecture slides). 
def NN_eval(input_x, submatrices):
	for i in range(len(submatrices)):
		a_x = []  # initialize as empty output
		# loop over the even indices of the submatrices array
		# even indices have weights
		b = submatrices[1] # get bias
		b_T = np.transpose(b) # transpose bias
		if i%2==0:
			w = submatrices[i]    # get weights
			w_T = np.transpose(w) # transpose weights
		# A(x) is activation function
		# matrix form:  A(X_w_01^T + b_1^T)
		# batch evaluation (multiple inputs --> multiple outputs)
		a_i = np.matmul(x, w_T) + b_T  # matrix multiplication
		a_x.append(a_i) # update output matrix
	return a_x
        

# 	x_x = input_x              # multiply by x_1...n s
# 	w = input_submatrices[0]   # weights
# 	b = input_submatrices[1]   # biases
# 	w_T = np.transpose(w)
# 	b_T = np.transpose(b)
# 	out = [] # create empty output
# 	for i in range(w.shape[1]):
# 		a_x = np.matmul(w_T, x) + b_T # activation function
# 		out.append(a_x)
# 	out = np.concatenate([a_x[:, None] for a_x in out], axis=1)
# 	return out
...
def loss(p,index_2_use):
	...
	# modify loss function for L1 Regularization (add abs values of fitting params)
	if(GAMMA_L1!=0.0):
		out=out+GAMMA_L1*np.sum(np.absolute(p))
	# modify loss function for L2 Regularization (add sum of sqs of fitting params)
	if(GAMMA_L2!=0.0):
